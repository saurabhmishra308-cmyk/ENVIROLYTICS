...
import { GaugeCircle, Plus, Pencil, Trash2, MailCheck, PlayCircle, AlertTriangle } from 'lucide-react';
    ...
    <Card data-testid="limits-card">
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        <div>
          ...
          <CardDescription>
            Each borewell can be capped at a max KL/month. When this month&apos;s draw exceeds the limit, the
            customer is emailed once automatically.
          </CardDescription>
        </div>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3" data-testid="limits-list">
            {items.map((it) => {
              const pct = it.monthly_limit_kl > 0
                ? Math.min(100, (it.consumption_kl_this_month / it.monthly_limit_kl) * 100)
                : 0;
              return (
                <div
                  key={it.hardware_id}
                  data-testid={`limit-row-${it.hardware_id}`}
                  className={`rounded-lg border p-3 ${it.exceeded ? 'border-red-300 bg-red-50 dark:bg-red-950/30' : 'bg-white dark:bg-gray-800'}`}
                >
                  ...
                  <div className="mt-3">
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className={it.exceeded ? 'text-red-600 font-semibold' : 'font-medium'}>
                        {Number(it.consumption_kl_this_month).toFixed(2)} / {Number(it.monthly_limit_kl).toFixed(2)} KL
                      </span>
                    </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Set the maximum monthly draw (KL) and the customer email to notify when crossed.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            ...
            <div><Label>Monthly limit (KL)</Label><Input type="number" min="0" value={form.monthly_limit_kl} onChange={(e) => setForm({ ...form, monthly_limit_kl: e.target.value })} data-testid="limit-create-kl" /></div>
        ...
        <DialogContent>
          ...
          {editTarget && (
            <div className="space-y-3 py-2">
              ...
              <div><Label>Monthly limit (KL)</Label><Input type="number" value={editTarget.monthly_limit_kl} onChange={(e) => setEditTarget({ ...editTarget, monthly_limit_kl: e.target.value })} data-testid="limit-edit-kl" /></div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="limit-edit-active" className="cursor-pointer">Active (send emails when exceeded)</Label>
              </div>
            </div>
          )}
          <DialogFooter>
