...
export const STPPlantDiagram = ({ values = {}, unit = 'mg/L', plantCapacityKld, deviceLabel, lastReceivedAt, stpUnitConfig = {}, stpDerived = {}, onEditConfig, canManage = false }) => {
...
export const Tank = ({ x, y, w, h, name, fillLevel = 0, capacityKld, accent = '#a16207' }) => {
...
export const PumpBadge = ({ x, y, label, flowKld }) => (
  <g transform={`translate(${x}, ${y})`}>
...
export const AirBlower = ({ x, y, label, capacity, powerKw }) => (
  <g transform={`translate(${x}, ${y})`}>
...
export const FeedPump = ({ x, y, label, flowKld, compact = false }) => (
  <g transform={`translate(${x}, ${y})`}>
    {/* KLD flow bubble above */}
...
export const FilterColumn = ({ x, y, label, color = '#3730a3' }) => (
  <g transform={`translate(${x}, ${y})`}>
...
export const STPProcessFlow = ({ values = {} }) => {
  const stages = [
...
// ------------------------- Admin: DO device → Tank assignment -------------------------
