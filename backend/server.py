...
from api_customer_profile import router as customer_profile_router
import api_customer_profile
from api_instrument_photos import router as instrument_photos_router
import api_instrument_photos
...
async def create_status_check(input: StatusCheckCreate, user: dict = Depends(auth_module.get_current_user)):
...
async def get_status_checks(user: dict = Depends(auth_module.get_current_user)):
...
async def startup_event():
    ...
    try:
        ...
        await db.flowmeter_readings.create_index([("hardware_id", 1), ("measurement_timestamp", -1)])
        await db.instrument_readings.create_index([("hardware_id", 1), ("timestamp", -1)])
        await db.instrument_readings.create_index([("hardware_id", 1), ("measurement_timestamp", -1)])
        await db.instrument_readings.create_index([("instrument_type", 1), ("hardware_id", 1), ("measurement_timestamp", -1)])
